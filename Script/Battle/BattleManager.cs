...
public class BattleManager : MonoBehaviour
{
    ...
    private void unitAttack()
    {
        ...
        if (ran <= battleCalculator.unitHitRate)
        {
            ...
            {
                if (skillManager.isGuard(battleCalculator.enemyDex, Random.Range(0f, 100.0f)))
                {
                    //小数点以下切り捨てでダメージ半減
            ...
            {
                if (skillManager.isDestinyGuard(battleCalculator.enemyLuk, Random.Range(0f, 100.0f)))
                {
                    //小数点以下切り捨てでダメージ半減
                    actualUnitAttack = Mathf.FloorToInt(actualUnitAttack / 2);
                }
            ...
            {
                //祈りが発動したら
                if (skillManager.isPray(battleCalculator.enemyLuk, ran))
                {
                    enemy.hp = 1;
                }
            ...
            if (unit.job.skills.Contains(Skill.メンテナンス))
            {
                ...
                if(skillManager.isMaintenance(battleCalculator.unitLuk, ran))
                {
                    //武器の消費が発生しないので何も無い
                }
                else
    ...
    private void enemyAttack()
    {
        ...
        if (ran <= battleCalculator.enemyHitRate)
        {
            ...
            {
                if (skillManager.isGuard(battleCalculator.unitDex, Random.Range(0f, 100.0f)))
                {
                    //小数点以下切り捨てでダメージ半減
            ...
            {
                if (skillManager.isDestinyGuard(battleCalculator.unitLuk, Random.Range(0f, 100.0f)))
                {
                    //小数点以下切り捨てでダメージ半減
                    actualEnemyAttack = Mathf.FloorToInt(actualEnemyAttack / 2);
                }
            ...
            {
                //祈りが発動したら
                if (skillManager.isPray(battleCalculator.unitLuk, Random.Range(0f, 100.0f)))
                {
                    unit.hp = 1;
                }
                //発生しなければそのままisDead判定で死ぬ

...
public class BattleTalkManager : MonoBehaviour
{
    ...
    //表示するテキストをセットするメソッド 会話前に呼んで使用する
    ...
    public void SetBattleStartTalk(Chapter chapter)
    {
    ...
    public void TalkEnd()
    {
        ...
        {
            battleMapManager.ChangeMapmodeTurnStart(MapMode.TURN_START);
        }
        else if (battleMapManager.mapMode == MapMode.TURN_START_TALK)
        {
            //ターン開始時会話は、開始エフェクト直後に挿入されているのでNORMALへ遷移
            battleMapManager.ChangeMapmodeTurnStart(MapMode.NORMAL);
        }
            ...
            battleMapManager.mapMode == MapMode.BATTLE_AFTER_TALK)
        {
            ...
            battleMapManager.ChangeMapmodeTurnStart(MapMode.BATTLE);
        }
    }
}

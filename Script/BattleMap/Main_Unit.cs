...
public class Main_Unit : MonoBehaviour
{
    ...
    public void MoveTo(Main_Cell cell)
    {
        ...
        var routeCells = map.CalculateRouteCells(x, y, moveAmount, cell, CalculationMode.PLAYER_MOVE);
    ...
    void OnClick()
    {
        ...
        if (isFocused)
        {
            //210219 武器を引数に渡さないと動作しなくなった もういらない
            //map.HighlightMovableCells(x, y, moveAmount.uni, CalculationMode.PLAYER_MOVE);
        }
        else
        {
            map.ResetMovableCells();
